...
object QueryUtils {
    ...
    fun extractEarthquakes(): ArrayList<QuakeData> {
        ...
        try {
            ...
            for (index in 0..10) {
                ...
                val Time = currentQuake.getString("time")
                val URL = currentQuake.getString("url")
                val DateData = splitDate(Time)
                val LocationData = splitLocation(Place)
                earthquakes.add(QuakeData(Magnitude, LocationData[0], LocationData[1], DateData[0], DateData[1], URL))
            }
    ...
    fun splitDate(inp: String): ArrayList<String> {
        val DateObj = Date(inp.toLong())
        val formatter0: SimpleDateFormat = SimpleDateFormat("MMM DD, yyyy")
        val formatter1: SimpleDateFormat = SimpleDateFormat("hh:mm a")
        formatter0.timeZone = TimeZone.getDefault()
        formatter1.timeZone = TimeZone.getDefault()

        return ArrayList<String>().apply {
            add(formatter0.format(DateObj))
            add(formatter1.format(DateObj))
        }
    }

    fun splitLocation(inp: String): ArrayList<String> {
        var relative: String = ""
        var fixed: String = ""

        val st: StringTokenizer = StringTokenizer(inp)
        while (st.hasMoreTokens()) {
            var currentToken = st.nextToken()
            fixed = "$fixed $currentToken"
            if (currentToken == "of") {
                relative = fixed
                fixed = ""
            }
